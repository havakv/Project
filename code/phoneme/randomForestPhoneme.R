...
err <- foreach(i = 1:length(B), .combine = c) %dopar% {
    ...
    fit <- randomForest(class ~ ., data = X$train, ntree = B[i])
...
Errors <- foreach(i = 1:length(mVec), .combine = cbind) %dopar% {
    ...
    err <- rep(NA, length(B))
    for (j in 1:length(B)) {
        fit <- randomForest(class ~ ., data = X$train, 
                            mtry = mVec[i], ntree = B[j])
        pred <- predict(fit, X$test, type="response")
        err[j] <- sum(pred != X$test$class)/X$nTest
    }
    cat("Ending", i, "\n")
    err
}
...
save(errRF, BRF, Errors, mVec, B, file = "../../dataset/phonemeResults/randomForestPhoneme.Rdata")
quit()
