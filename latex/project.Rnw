...
Something about the project and thanks to Håvard Rue.

\clearpage
\begin{abstract}
Write something interesting here!!!!! And make the margins larger (text more narrow).
\end{abstract}
...
%
\let\cleardoublepage\clearpage
\chapter{Empirical experiments}
\colorbox{yellow}{Numerical experiments?}\\
<<simulations, child='./simulations.tex', cache=TRUE>>=
@
